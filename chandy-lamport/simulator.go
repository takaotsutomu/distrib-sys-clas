...
type Simulator struct {
	...
	logger           *Logger
	snapshotChannels *SyncMap // map[int]chan *Snapshot, where key = snapshotId
}
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	sim.snapshotChannels.Store(snapshotId, make(chan *Snapshot))
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	value, _ := sim.snapshotChannels.Load(snapshotId)
	snapReady := value.(chan *Snapshot)
	snapReady <- sim.servers[serverId].Snapshots[snapshotId]
}
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	...
	for i := 0; i < len(sim.servers); i++ {
		localSnap := <-snapReady
		snap.tokens[localSnap.ServerId] = localSnap.Tokens
		snap.messages = append(snap.messages, localSnap.Messages...)
	}
	sim.snapshotChannels.Delete(snapshotId)
	return &snap
}
